...
_mqtt_restart:
    ...
    if (c->isblocking && c->pub_sem)//这里加上释放判断，下面没有释放操作了，之后重连会一直无法推送，等待信号量超时
    {
        if(c->pub_sem->value == 0)
        {
            LOG_D("release pub_sem");
            rt_sem_release(c->pub_sem);
        }
        else
        {
            LOG_D("pub_sem value is %d,do not release it",c->pub_sem->value);
        } 
    }
    net_disconnect(c);
